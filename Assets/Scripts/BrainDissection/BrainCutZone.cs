...
/// <summary>
/// Invisible trigger zone at the brain's sagittal midline.
///
/// The zone is intentionally WIDE (not paper-thin) so the knife
/// can realistically pass through it. The user must run the knife
/// through enough of the zone's height (requiredCutFraction) to
/// trigger the split.
///
/// At Start the BoxCollider is automatically widened to 10cm in
/// world X so that it's easy to hit from any approach angle.
/// </summary>
...
public class BrainCutZone : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (bc != null)
        {
            // ---- AUTO-WIDEN the collider so the knife can actually hit it ----
            // Make the X dimension at least 0.10 world units (~10 cm)
            ...
            // Cache the zone height in world space
