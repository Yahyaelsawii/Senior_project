                ...
                s_BuildValidationRules.Add(
                    new BuildValidationRule
                    {
                        ...
                        CheckPredicate = () => 
                        {
                            ...
                            try
                            {
                                // Check if InputSystem.actions is available (requires Input System 1.11.0+)
                                var actionsProperty = typeof(InputSystem).GetProperty("actions");
                                if (actionsProperty == null)
                                    return true; // Property doesn't exist in this version
                                
                                var actionsValue = actionsProperty.GetValue(null);
                                return actionsValue == null || PackageVersionUtility.GetPackageVersion(k_InputSystemPackageName) >= s_RecommendedPackageVersion;
                            }
                            catch
                            {
                                return true; // InputSystem.actions not available in this version
                            }
#else
                            return true;
#endif
